...
use lst_optimizer_client::{ app::OptimizerApp, pool::{ MaxPool, MaxPoolOptions } };
...
async fn main() {
    ...
    let pool = MaxPool::new(&program_id, MaxPoolOptions {
        rpc_url,
        ..Default::default()
    });

    let err = OptimizerApp::new(pool).keep_rebalance(
        context
